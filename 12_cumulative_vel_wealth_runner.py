    ...
    for _key in velocities[0].keys():
        for _count in range(0,len(velocities[0][_key])):
            if(velocities[0][_key][_count]>0):
                list_velocities[_count][_key]=velocities[0][_key][_count]/velocities[1][_key][_count]
pickle.dump(list_velocities,open('campajola_velocities_wealth_new.pickle','wb'))
print('Done')
